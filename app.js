(() => {
  ...
  async function ensureForRoute(route){
    const idsByType=t=>app.catalog.packs.filter(p=>p.type===t).map(p=>p.id);
    if(route==='home') await Promise.all(['kanji-core',...idsByType('dialogue').filter(id=>id!=='dialogues-daily')].map(loadPack));
    if(route==='vocab'||route==='review') await Promise.all(['kanji-core',...idsByType('vocabulary')].map(loadPack));
    if(route==='dialogue') await Promise.all(idsByType('dialogue').map(loadPack));
    if(route==='drill') await Promise.all([...idsByType('grammar'),...idsByType('mistake')].map(loadPack));
  }
  ...
  function renderVocab(){const q=app.search.trim().toLowerCase();const filtered=groupArray().filter(g=>{const ws=(g.wordIds||[]).map(id=>app.words.get(id)).filter(Boolean);const text=[g.kanji,g.meaningsZh.join(' '),...ws.flatMap(w=>[w.word,w.reading,...w.meaningsZh,...w.tags])].join(' ').toLowerCase();const mf=app.vocabFilter==='all'||ws.some(w=>(w.jlptLevel===app.vocabFilter)||w.tags.some(t=>t.toLowerCase()===app.vocabFilter.toLowerCase()));return (!q||text.includes(q))&&mf;});main.innerHTML=`<div class="search-row"><input id="vocab-search" class="search-input" value="${escapeHtml(app.search)}" placeholder="搜索汉字、单词、读音或标签"/><button class="ghost-button" data-action="clear-search">清除</button></div><div class="filter-pills">${[['all','全部'],['基础','基础'],['JLPT','JLPT'],['日常','日常'],['职场','职场'],['工作','商务'],['IT','IT'],['Git','Git'],['CI/CD','CI/CD'],['AWS','AWS'],['N2','N2'],['N1','N1']].map(([v,l])=>`<button class="pill ${app.vocabFilter===v?'active':''}" data-vocab-filter="${v}">${l}</button>`).join('')}</div><div class="card-grid">${filtered.length?filtered.map(kanjiCard).join(''):emptyState('没有匹配内容','换一个关键词试试。')}</div>`;}
  ...
  function renderDialogues(){const ds=dialogueArray().filter(d=>app.dialogueFilter==='all'||(app.dialogueFilter==='daily'?d.category==='daily':(app.dialogueFilter==='N2'||app.dialogueFilter==='N1'?d.jlptLevel===app.dialogueFilter:d.category!=='daily')));main.innerHTML=`<div class="filter-pills"><button class="pill ${app.dialogueFilter==='all'?'active':''}" data-dialogue-filter="all">全部</button><button class="pill ${app.dialogueFilter==='daily'?'active':''}" data-dialogue-filter="daily">日常生活</button><button class="pill ${app.dialogueFilter==='business'?'active':''}" data-dialogue-filter="business">商务 / IT / 职场</button><button class="pill ${app.dialogueFilter==='N2'?'active':''}" data-dialogue-filter="N2">N2</button><button class="pill ${app.dialogueFilter==='N1'?'active':''}" data-dialogue-filter="N1">N1</button></div><div class="card-grid two">${ds.map(dialogueSummaryCard).join('')}</div>`;}
