const APP_CACHE = 'nihongo-lab-app-v4.0.0';
const CONTENT_CACHE = 'nihongo-lab-content-2026.07.31-phase1-business-it';
...
self.addEventListener('fetch', event => {
  ...
  if(url.pathname.endsWith('/data/catalog.json')) {
    event.respondWith(fetch(event.request).then(r=>{const copy=r.clone();caches.open(CONTENT_CACHE).then(c=>c.put(event.request,copy));return r;}).catch(()=>caches.match(event.request)));
    return;
  }
  if(url.pathname.includes('/data/packs/') || url.pathname.includes('/schemas/')) {
    event.respondWith(caches.match(event.request).then(cached=>{const network=fetch(event.request).then(r=>{if(r.ok){const copy=r.clone();caches.open(CONTENT_CACHE).then(c=>c.put(event.request,copy));}return r;});return cached||network;}));
    return;
  }
